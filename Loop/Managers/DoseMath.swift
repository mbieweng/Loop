...
extension InsulinCorrection {
    ...
    fileprivate func asTempBasal(
        ...
        if(UserDefaults.appGroup.autoSensFactor > 1.15 || glucVal < 120 ) {
...
private func zeroTempEffect(percentEffectDuration: Double) -> Double {
    ...
    let td = 330.0 // set to td = DIA = 360 min nominally for exponential curves
...
private func targetGlucoseValue(percentEffectDuration: Double,
    ...
    if initialValue < minValue && glucoseValue > 120.0 && UserDefaults.appGroup.autoSensFactor < 1.15 {
